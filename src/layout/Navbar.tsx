      ...
      >
        <div className="container-custom h-full flex items-center justify-between relative">
          ...
          <Link to="/" className={`transition-all duration-300 hover:opacity-90 hover:-translate-y-0.5 ${isHero ? 'absolute left-4 md:left-8 top-4 md:-top-4 z-[110]' : 'flex items-center z-50'}`}>
            <Logo
              className={`transition-all duration-500 rounded-full border-[5px] border-white overflow-hidden bg-white shadow-[0_8px_20px_rgba(0,0,0,0.2)] flex justify-center items-center ${isHero
              ...
              <React.Fragment key={link.name}>
                <Link
                  ...
                  className={`font-barlow text-[13px] md:text-[14px] font-700 uppercase tracking-widest transition-all duration-300 px-3 py-1 rounded relative group ${currentPath === link.path
                    ? 'text-[#C0392B]'
                    : 'text-black hover:text-[#C0392B]'
                    }`}
                >
                  {link.name}
                  <span className={`absolute bottom-0 left-3 right-3 h-[2px] bg-[#C0392B] transition-all duration-300 ${currentPath === link.path ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'
                    }`} />
                </Link>
          ...
          <div className="flex items-center gap-4 md:gap-6 ml-auto relative z-[120]">
            <a
              href="tel:0455123678"
              className="hidden md:flex items-center gap-2 text-black hover:text-[#C0392B] transition-colors duration-300 group"
            >
            ...
            <Link
              to="/menu"
              className="hidden sm:flex bg-[#C0392B] text-white hover:bg-[#A93226] font-barlow font-800 tracking-wider text-[15px] px-6 py-2.5 rounded-sm shadow-md transition-all duration-300"
            >
